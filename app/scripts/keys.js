const Keys = {
    ...
    init() {
        ...
        window.addEventListener(
            'keydown',
            event => this._onKey(event, true)
        );
        window.addEventListener(
            'keyup',
            event => this._onKey(event, false)
        );
    },

    startNote(ID, key) {
        this.ACTIVE_NOTE_IDS[key] = ID;
    },

    finishNote(key) {
        const ID = this.ACTIVE_NOTE_IDS[key];
        delete this.ACTIVE_NOTE_IDS[key];
        return ID;
    },
    ...
    _renderKeys() {
        let notes = [];
        for (let i = 0; i < 12; i++) {
            notes.push(i ? notes[i - 1] * 1.5 : this.BASE_FREQ);
        }
        notes = notes.map(n => {
            while (n >= notes[0] * 2) {
                n *= 0.5;
        ...
        notes = [
            ...
            .map((n, ni, nArr) => ({
                ...
                shortcut: [
                    this.SHORTCUTS[0][ni],
                    this.SHORTCUTS[1][nArr.length - ni - 1]
                ].filter(s => s)
            }));
    ...
    _onKey(event, isDown) {
        ...
        let
            ...
            elKey = null;
        if (key && this.SHORTCUTS.flat().indexOf(key) !== -1) {
